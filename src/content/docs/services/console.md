...
The admin console as a service. The operator-facing manual, covering every page, the
auth model and what the console can and cannot do, is
...
The console refuses to start over an unreadable or invalid manifest, and the
...
`unreachable`, so a down registry or issuer degrades exactly that page's

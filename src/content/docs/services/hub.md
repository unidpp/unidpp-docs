...
that declines interop is never brokered around — the WILL gap is not
...
admin surface — nothing to administer.
...
reason — an L0 posture (`WILL gap`) or an absent declaration (the
silent side named).
...
Backups: nothing to back up. Upgrades: restart — the hub held
